...
import 'package:easyrent/features/rentee/renting_status/presentation/widgets/return_verification_dialog.dart';
...
class _RentalItemCardWidgetState extends State<RentalItemCardWidget> {
  ...
  // Show return verification dialog with QR code
  void _showReturnVerificationDialog(BuildContext context) {
    showDialog(
      context: context,
      builder:
          (context) => ReturnVerificationDialog(
            orderId: widget.item.id,
            itemName: widget.item.productName,
            itemCode: widget.item.id,
          ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: InkWell(
        onTap: () {
          _showReturnVerificationDialog(context);
        },
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        GestureDetector(
                          onTap: () {
                            // Copy return date to clipboard
                            Clipboard.setData(
                              ClipboardData(text: formattedReturnDate),
                            );
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(
                                  'Return date copied: $formattedReturnDate',
                                ),
                              ),
                            );
                          },
